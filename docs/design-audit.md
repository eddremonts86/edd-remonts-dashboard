...
The Contact section's inverted background (`bg-foreground`) makes it a deliberate contrast moment, so the bolder typographic choice _could_ be intentional. If keeping the bold uppercase, at least make the main title weight consistent with other sections (`font-light` instead of `font-black`), using weight contrast for hierarchy rather than doubling font-black + uppercase.
