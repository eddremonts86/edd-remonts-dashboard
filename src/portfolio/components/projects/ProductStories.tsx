...
const TOP_STORIES: StoryProject[] = [
  {
    ...
    businessContext: 'Enterprise publishing platform serving large media organizations across Europe with high data-density requirements.',
    problem: 'Initial bundle size exceeded 6MB, causing LCP delays over 7.5s and high load abandonment rates.',
    constraints: 'Active multi-decade legacy codebase. No complete core platform rewrite allowed due to strict regression risks.',
    solution: 'Designed modular Vite-based monorepo partitions and enabled route isolation, pinning sub-apps to strict semantic contracts.',
    outcome: 'Initial bundle payload reduced by 94.3% (6.2MB to 350KB). LCP improved to 1.4s, and delivery cycle speeds rose by 30%.',
  ...
  },
  {
    ...
    businessContext: 'Transactional control dashboard managing real-time logistics tracking and tracking KPIs with sub-second accuracy.',
    problem: 'High-throughput stream polling saturated client sockets, while direct WebSocket piping caused heavy DOM thrashing.',
    constraints: 'Unstable network sockets on client edges. Must run seamlessly at a steady 60FPS on commercial viewports.',
    solution: 'Engineered event-driven virtualized DOM memoization queues coupled with TanStack Query optimistic synchronization.',
    outcome: 'Slashing server requests by 60%, handles 500+ updates per second with zero frame drops, securing a 98 Lighthouse rating.',
  ...
  },
  {
    ...
    businessContext: 'Full-stack site builder assembler focused on modular layout templates and fast static regeneration.',
    problem: 'Real-time canvas assembly of hundreds of elements triggered global re-render cascades, spiking interaction delay (INP).',
    constraints: 'Must compile custom modular code templates on the fly while enforcing a sub-12ms key-response budget.',
    solution: 'Adopted a composite UI structure where layout nodes operate as independent reactive registers, isolating mutations.',
    outcome: 'Maintained a 100% Core Web Vitals score. Input response latency remains strictly under 12ms during complex layout edits.',
