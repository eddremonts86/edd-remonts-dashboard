    ...
    <div className="rounded-xl border border-subtle bg-surface/50 p-5 font-mono text-[9.5px] space-y-4 shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <span className="text-green-600 font-bold block">{t('projects.stories.pathChosen', '✓ PATH CHOSEN')}</span>
          <p className="text-foreground/70 leading-relaxed font-light font-sans text-xs">
            {chosen}
          </p>
        </div>
        <div className="space-y-1.5">
          <span className="text-red-500 font-bold block">{t('projects.stories.pathRejected', '✗ PATH REJECTED')}</span>
          <p className="text-foreground/70 leading-relaxed font-light font-sans text-xs">
            {rejected}
          </p>
        </div>
...
export const ProductStories = () => {
  const { t } = useTranslation();
  const [modalState, setModalState] = useState<{ projectId: string; view: 'interface' | 'architecture' } | null>(null);
  ...
  const topStories: StoryProject[] = [
    {
      ...
      context: t('projects.stories.schilling.context', 'Enterprise publishing SaaS serving large media organizations across Europe with high-data-density requirements.'),
      challenge: t('projects.stories.schilling.challenge', 'Monolithic bundle sizes exceeded 6MB, which triggered LCP delays over 7.5s, raised bounce rates, and caused high-load abandonment. Complete core platform rewrites were strictly forbidden due to legacy billing and data-structure regression risks.'),
      decision: t('projects.stories.schilling.decision', 'Rather than freeze features for a multi-year rewrite, I designed Vite-based monorepo partitions, enabled route isolation, and established strict semantic contracts between modules, allowing product teams to build and ship independently.'),
      businessImpact: t('projects.stories.schilling.businessImpact', 'Slashed initial bundle payloads by 94% (6.2MB to 350KB), which dropped customer abandonment to near-zero, accelerated feature-delivery speeds by 30%, and allowed 20+ engineers to ship features with zero regressions.'),
      tradeoffsChosen: t('projects.stories.schilling.tradeoffsChosen', 'Scoped monorepo sub-app partitions with strict semantic-version contracts to support independent, low-risk releases.'),
      tradeoffsRejected: t('projects.stories.schilling.tradeoffsRejected', 'Monolithic rewrite, avoiding a 12-month feature freeze and multi-million-euro regression risks.'),
    ...
    },
    {
      ...
      architectureLabel: t('projects.stories.edd-remonts.architectureLabel', 'TanStack DB Synchronization'),
      urlPlaceholder: 'logistics.remonts.io/dashboard',
      coverPath: '/projects/edd-remonts-cover.png',
      context: t('projects.stories.edd-remonts.context', 'Real-time logistics tracking and transactional control dashboard managing high-frequency operational KPIs.'),
      challenge: t('projects.stories.edd-remonts.challenge', 'Constant high-frequency data polling saturated client sockets, triggered heavy server-side load, and caused severe page-load latency. Direct WebSockets caused massive DOM thrashing and layout shifts on unstable mobile networks.'),
      decision: t('projects.stories.edd-remonts.decision', 'Engineered virtualized-DOM memoization queues, integrated TanStack Query for optimistic caching, and isolated rendering updates strictly to active cells instead of re-rendering whole list panels.'),
      businessImpact: t('projects.stories.edd-remonts.businessImpact', 'Slashed socket query traffic by 60%, reduced server-infrastructure costs, and secured a perfect 98 Lighthouse score. The application runs at a steady 60FPS on commercial-grade mobile devices even under intensive real-time transactional streams (500+ updates/sec).'),
      tradeoffsChosen: t('projects.stories.edd-remonts.tradeoffsChosen', 'Optimistic state synchronization coupled with a 150ms virtualized-DOM throttle to isolate CPU load.'),
      tradeoffsRejected: t('projects.stories.edd-remonts.tradeoffsRejected', 'Raw WebSockets under 10ms, which would saturate mobile sockets and cause clientside layout shifts.'),
    ...
    },
    {
      ...
      context: t('projects.stories.zunzun.context', 'Full-stack canvas-builder and layout-assembler focused on modular templates and fast static-site regeneration.'),
      challenge: t('projects.stories.zunzun.challenge', 'Canvas assembly of hundreds of elements triggered global state-re-render cascades, spiked key interaction delays (INP), and degraded user experiences.'),
      decision: t('projects.stories.zunzun.decision', 'Re-architected state flow to use localized reactive nodes, treated layout elements as independent registers, and completely isolated visual mutations from the global state tree to prevent cascades.'),
      businessImpact: t('projects.stories.zunzun.businessImpact', 'Guaranteed sub-12ms response times, maintained a 100% Core Web Vitals score across all outputs, and eliminated drop-offs during intensive real-time editing.'),
      tradeoffsChosen: t('projects.stories.zunzun.tradeoffsChosen', 'Strict composite-UI layout nodes operating as independent registers to isolate mutations to leaves.'),
      tradeoffsRejected: t('projects.stories.zunzun.tradeoffsRejected', 'Single global-state dispatch, avoiding global re-render cascades that spike key input latency (INP).'),
          ...
          >
            ...
            <div className="grid gap-12 lg:grid-cols-12 lg:gap-16 items-start">
              ...
              <div className={`lg:col-span-6 space-y-6 ${isOdd ? 'lg:order-2' : ''}`}>
                ...
                <div className="flex border border-subtle rounded-xl bg-surface/50 p-1 divide-x divide-subtle font-mono text-[9px] tracking-wider relative select-none">
                  <button
                    ...
                    className="flex-1 py-3 text-center rounded-lg cursor-pointer transition-colors flex items-center justify-center gap-1.5 font-bold bg-transparent border border-transparent text-foreground/50 hover:text-foreground hover:bg-foreground/[0.02] w-full"
                  >
                ...
                <div className="rounded-xl border border-subtle bg-surface/30 p-5 space-y-4 font-mono text-[10px] text-left">
                  ...
                  {project.architectureDiagramType === 'microfrontend' && (
                    <div className="space-y-3 font-sans">
                      <div className="flex justify-between items-baseline gap-2">
                        <span className="font-semibold text-foreground">{t('projects.stories.outcomePayload', '42% Payload Reduction [6.2MB → 350KB]')}</span>
                      </div>
                      <p className="text-[10px] text-foreground/60 leading-relaxed font-light">
                        {t('projects.stories.outcomePayloadDesc', 'Vite-based monorepo partitions decoupled legacy assets, securing a perfect 1.4s LCP load time.')}
                      </p>
                    </div>
                  )}
                  ...
                  {project.architectureDiagramType === 'statesync' && (
                    <div className="space-y-3 font-sans">
                      <div className="flex justify-between items-baseline gap-2">
                        <span className="font-semibold text-foreground">{t('projects.stories.outcomeDeduplication', '60% Request Deduplication [500+ updates/sec]')}</span>
                      </div>
                      <p className="text-[10px] text-foreground/60 leading-relaxed font-light">
                        {t('projects.stories.outcomeDeduplicationDesc', 'Virtualized-DOM rendering throttles prevented mobile thrashing and cut server-infrastructure costs.')}
                      </p>
                    </div>
                  )}
                  ...
                  {project.architectureDiagramType === 'composition' && (
                    <div className="space-y-3 font-sans">
                      <div className="flex justify-between items-baseline gap-2">
                        <span className="font-semibold text-foreground">{t('projects.stories.outcomeVitals', '100% Core Web Vitals [INP Sub-12ms]')}</span>
                      </div>
                      <p className="text-[10px] text-foreground/60 leading-relaxed font-light">
                        {t('projects.stories.outcomeVitalsDesc', 'Localized reactive leaf nodes completely isolated visual mutations from the global state tree.')}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className={`lg:col-span-6 space-y-6 text-left ${isOdd ? 'lg:order-1' : ''}`}>
                ...
                <TradeoffLedger chosen={project.tradeoffsChosen} rejected={project.tradeoffsRejected} />
              </div>
              ...
              <div className="flex items-center justify-between border-b border-subtle pb-3">
                <div>
                  ...
                  <span className="text-[9px] uppercase tracking-wider text-primary font-bold font-mono">
                    / {modalState.view === 'interface' ? t('projects.stories.modal.interfaceCapture', 'Interface Viewport Capture') : t('projects.stories.modal.topologySchema', 'Workspace Topology Schema')}
                  </span>
                </div>
                <button
                  onClick={() => setModalState(null)}
                ...
                {modalState.view === 'interface' ? (
                  ...
                  <div className="w-full h-full p-8 flex flex-col justify-between select-none">
                    ...
                    <div className="flex items-center justify-between border-b border-subtle pb-3">
                      ...
                      <span className="font-mono text-[8px] text-foreground/35">{t('projects.stories.modal.diagnostic', 'DIAGNOSTIC_SNAP')}</span>
                    </div>
                    ...
                    {activeProject.architectureDiagramType === 'microfrontend' && (
                      <svg className="w-full h-60 my-auto" viewBox="0 0 200 120" fill="none">
                        ...
                        <text x="100" y="22" fill="var(--primary)" fontSize="6" fontFamily="monospace" textAnchor="middle">{t('projects.stories.modal.workspace', 'MONOREPO WORKSPACE [SCHILLING]')}</text>
                        ...
                        <text x="100" y="97.5" fill="var(--primary-foreground)" fontSize="7" fontWeight="bold" fontFamily="monospace" textAnchor="middle">VITE</text>
                      </svg>
                    )}
                    ...
                    {activeProject.architectureDiagramType === 'statesync' && (
                      <svg className="w-full h-60 my-auto" viewBox="0 0 200 120" fill="none">
                        ...
                        <text x="100" y="10" fill="var(--primary)" fontSize="5" fontFamily="monospace" textAnchor="middle">{t('projects.stories.modal.stream', 'LIVE TRANSACTIONAL OPERATIONS STREAM')}</text>
                    ...
                    {activeProject.architectureDiagramType === 'composition' && (
                      <svg className="w-full h-60 my-auto" viewBox="0 0 200 120" fill="none">
                        ...
                        <text x="100" y="22.5" fill="var(--primary)" fontSize="5.5" fontFamily="monospace" textAnchor="middle">{t('projects.stories.modal.store', 'Global Store')}</text>
                        ...
                        <text x="57.5" y="70" fill="var(--foreground)" fontSize="5" fontFamily="monospace" textAnchor="middle">{t('projects.stories.modal.registry', 'Client Registry')}</text>
                        <rect x="120" y="60" width="45" height="18" rx="2" fill="var(--surface)" stroke="var(--border-default)" strokeWidth="0.5" />
                        <text x="142.5" y="70" fill="var(--foreground)" fontSize="5" fontFamily="monospace" textAnchor="middle">{t('projects.stories.modal.leaf', 'Isolated Leaf')}</text>
