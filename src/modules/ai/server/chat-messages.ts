  ...
  return rawMessages
    .map((message) => {
      ...
      const parts: ChatMessage['parts'] = message.parts.map((part) => {
        ...
        if (part.type === 'image') {
          // TanStack AI sends: { type: 'image', source: { type: 'url'|'data', value: string } }
          // Legacy format: { type: 'image', image: string }
          const rawPart = part as unknown as Record<string, unknown>
          const imageUrl =
            (rawPart.source as { value?: string } | undefined)?.value ??
            (rawPart.image as string | undefined) ??
            ''
