...
/** Split raw text content that may embed <think>...</think> blocks inline */
function parseThinkBlocks(raw: string): Array<{ type: 'think' | 'text'; content: string }> {
  const result: Array<{ type: 'think' | 'text'; content: string }> = []
  ...
  while ((m = re.exec(raw)) !== null) {
    ...
    if (thinking) result.push({ type: 'think', content: thinking })
    last = m.index + m[0].length
  }
  const tail = raw.slice(last).trim()
  if (tail) result.push({ type: 'text', content: tail })
  // If nothing matched, return the original as-is
  if (result.length === 0 && raw.trim()) result.push({ type: 'text', content: raw })
  return result
}
...
function ThinkingProcess({ content }: { content: string }) {
  ...
  return (
    <m.div
      ...
      className="overflow-hidden rounded-xl border border-indigo-500/15 bg-indigo-500/4 transition-colors duration-200 hover:bg-indigo-500/[0.07]"
    >
      <button
      ...
      >
        ...
        >
          <Sparkles size={11} className="shrink-0 text-indigo-400/60" />
        </m.div>
        <span className="text-[11px] font-medium tracking-wide text-indigo-400/70 dark:text-indigo-400/60">
          {t('ai.chat.thinking')}
        </span>
        <m.span
          className="ml-auto"
      ...
      <AnimatePresence initial={false}>
        {isExpanded && (
          ...
          >
            <div className="border-t border-indigo-500/10 px-4 py-3 font-mono text-[11px] leading-relaxed text-muted-foreground/60 whitespace-pre-wrap">
              {content}
            </div>
          </m.div>
        )}
...
const MarkdownA = ({ children, href }: { children?: React.ReactNode; href?: string }) => (
  <a
    ...
    className="font-medium text-indigo-400 underline decoration-indigo-400/40 underline-offset-2 transition-colors hover:text-indigo-300 hover:decoration-indigo-300/70"
  >
    {children}
  </a>
)
const MarkdownBlockquote = ({ children }: { children?: React.ReactNode }) => (
  <blockquote className="my-3 rounded-r-lg border-l-2 border-indigo-400/40 bg-indigo-500/4 py-2 pl-4 italic text-muted-foreground/80">
    {children}
  </blockquote>
)
const MarkdownTable = ({ children }: { children?: React.ReactNode }) => (
  <div className="my-4 w-full overflow-x-auto rounded-xl border border-white/8">
    <table className="w-full text-sm">{children}</table>
  </div>
)
const MarkdownTh = ({ children }: { children?: React.ReactNode }) => (
  <th className="border-b border-white/8 bg-white/4 px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground/70">
    {children}
  </th>
)
...
const MarkdownCode = ({
  ...
  ref: _ref,
  ...props
}: React.ComponentPropsWithoutRef<'code'> & { ref?: React.Ref<HTMLElement> }) => {
  const match = /language-(\w+)/.exec(className || '')
  if (match?.[1] === 'action') {
    return <ActionConfirmationCard actionJson={String(children).replace(/\n$/, '')} />
  }
  if (!String(className).includes('inline') && match) {
    return (
      <div className="my-3 overflow-hidden rounded-xl border border-white/[0.07] bg-zinc-950 shadow-lg">
        <div className="flex items-center justify-between border-b border-white/5 bg-white/2.5 px-4 py-1.5">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-6 w-6 rounded text-zinc-600 hover:bg-white/10 hover:text-zinc-300"
  ...
  return (
    <code
      className="rounded-md bg-white/9 px-1.5 py-0.5 font-mono text-[0.82em] text-foreground/85"
      {...props}
    >
      {children}
    </code>
...
// Unified AI message markdown renderer
...
function MessageBubble({
  ...
  return (
    <m.div
      ...
      className={cn('group flex w-full items-end gap-3', isUser ? 'justify-end' : 'justify-start')}
    >
      {/* AI avatar — anchored to bubble bottom */}
      {!isUser && (
        <div className="mb-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-indigo-500 to-violet-600 shadow-md shadow-indigo-500/20">
      ...
      <div
        className={cn(
          'flex flex-col gap-2',
          isUser ? 'max-w-[68%] items-end' : 'max-w-[86%] items-start',
        )}
      >
        {/* Meta row */}
        <div className={cn('flex items-center gap-1.5 px-1', isUser && 'flex-row-reverse')}>
          ...
          {!isUser && (
            <button
              ...
              className="ml-0.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100"
              title="Copy response"
            >
              {copied ? (
        ...
        <div
          className={cn(
            ...
            isUser
              ? 'rounded-3xl rounded-tr-sm bg-linear-to-br from-violet-600 to-indigo-500 px-5 py-3.5 text-white shadow-lg shadow-indigo-500/20'
              : 'rounded-2xl rounded-tl-sm border border-white/8 bg-white/4 px-6 py-5 shadow-sm backdrop-blur-sm dark:border-white/6 dark:bg-white/3',
          )}
        >
          {Array.isArray(message.parts)
            ? message.parts.map((part, partIndex) => {
                ...
                if (part.type === 'text') {
                  if (!part.content?.trim()) {
                    if (!isUser) {
                      return (
                        <div
                          key={partKey}
                          className="flex items-center gap-2 rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-600 dark:text-amber-400"
                        >
                          <span>⚠️</span>
                  ...
                  if (!isUser) {
                    const blocks = parseThinkBlocks(part.content)
                    if (blocks.some((b) => b.type === 'think')) {
                      return (
                        <div key={partKey} className="flex flex-col gap-2">
                          {blocks.map((block, idx) =>
                            block.type === 'think' ? (
                              <ThinkingProcess key={`${partKey}-t${idx}`} content={block.content} />
                            ) : (
                              <MarkdownContent key={`${partKey}-m${idx}`} content={block.content} />
                            ),
                          )}
                        </div>
                      )
                    }
                    return <MarkdownContent key={partKey} content={part.content} />
                  }

                  // User message — strip any stale <think> blocks then render
                  const userBlocks = parseThinkBlocks(part.content)
                  ...
                  return (
                    <div key={partKey} className="text-sm leading-relaxed text-white/90">
                      <ReactMarkdown
                        remarkPlugins={[remarkGfm]}
                        components={{
                          p: ({ children }) => (
                          ...
                          code: ({ className, children, ...props }) => {
                            const match = /language-(\w+)/.exec(className || '')
                            return match ? (
                              <pre className="my-2 overflow-x-auto rounded-xl bg-black/20 px-4 py-3 font-mono text-xs">
                                <code>{children}</code>
                              </pre>
                            ) : (
                              <code
                                className="rounded bg-white/20 px-1.5 py-0.5 font-mono text-[0.82em]"
                                {...props}
                              >
                                {children}
                              </code>
                ...
                if (part.type === 'image') {
                  ...
                  return (
                    <div
                      key={partKey}
                      className="mt-2 overflow-hidden rounded-xl border border-white/10 shadow-sm"
                    >
                      <button
                        type="button"
                      ...
                      >
                        <img
                          ...
                          className="max-h-64 w-full cursor-zoom-in object-cover transition-transform duration-300 hover:scale-105"
                        />
                      </button>
                    </div>
                  )
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        Using tool: <span className="font-mono">{toolName}</span>
                      </div>
                    </div>
                  )
                }
          ...
          {isTyping && (
            <div className="flex items-center gap-1.5 py-0.5">
              <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-indigo-400/60 [animation-delay:-0.3s]" />
              <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-indigo-400/60 [animation-delay:-0.15s]" />
              <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-indigo-400/60" />
            </div>
          )}
        </div>
      </div>

      {/* User avatar — anchored to bubble bottom */}
      {isUser && (
        <div className="mb-0.5 shrink-0">
          {userAvatar ? (
            <Avatar className="h-8 w-8 rounded-full ring-2 ring-violet-500/25">
              <AvatarImage src={userAvatar} alt="User" />
              <AvatarFallback className="bg-linear-to-br from-violet-500 to-indigo-500 text-xs text-white">
                <User size={14} />
              </AvatarFallback>
            </Avatar>
          ) : (
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-violet-500/20 to-indigo-500/20 ring-2 ring-white/10 dark:ring-white/6">
              <User size={14} className="text-indigo-400/70" />
            </div>
          )}
        </div>
      )}
      ...
      <div className="grid w-full max-w-2xl grid-cols-1 gap-3 sm:grid-cols-2">
        {suggestions.map((s, i) => (
          <m.button
            ...
            className="group relative overflow-hidden rounded-2xl border border-border/40 bg-card p-5 text-left shadow-sm transition-colors duration-300 hover:border-indigo-500/30 hover:bg-indigo-500/5"
          >
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <span className="text-xs text-muted-foreground mt-0.5 block">{s.desc}</span>
              </div>
            </div>
...
function isDirectTextFile(file: File): boolean {
  ...
  if (DIRECT_TEXT_EXTENSIONS.has(ext)) return true
...
async function extractDocumentText(file: File): Promise<string> {
  try {
    ...
    if (!res.ok) {
      ...
      return `[File: ${file.name}]\nError extracting content: ${err.error ?? res.statusText}`
    }
...
export function HelpChatPage() {
  ...
  const handleSend = async (overrideContent?: string) => {
    ...
    const hasBinaryFiles = attachments.some(
      (f) => !f.type.startsWith('image/') && !isDirectTextFile(f),
    )
    if (hasBinaryFiles) setIsParsing(true)
    ...
    if (!trimmed && attachments.length > 0) {
      const hasDocuments = attachments.some((f) => !f.type.startsWith('image/'))
      const instruction = hasDocuments
    ...
    setInput('')
    setAttachments([])
    // Reset file input so the same file can be re-selected after sending
      ...
      <LazyMotion features={domAnimation}>
        <div className="relative flex h-full flex-col overflow-hidden rounded-[2rem] border border-white/20 bg-background/50 shadow-2xl backdrop-blur-2xl dark:border-white/5 dark:bg-black/40">
          ...
          <m.header
            ...
            className="flex h-20 items-center justify-between border-b border-white/10 bg-white/40 px-8 backdrop-blur-md dark:bg-black/20"
          >
          ...
          <div
            ...
            className="flex-1 overflow-y-auto scroll-smooth p-8 md:p-10 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-indigo-500/10 hover:scrollbar-thumb-indigo-500/20"
          >
            <AnimatePresence mode="wait">
              {visibleMessages.length === 0 && !error ? (
                <EmptyState key="empty" onSuggestionClick={handleSend} />
              ) : (
                <m.div
                  key="messages"
                  initial={{ opacity: 0 }}
                  ...
                  className="flex flex-col gap-7 mx-auto max-w-3xl"
                >
                  ...
                  <AnimatePresence>
                    {error && !isLoading && (
                      ...
                      >
                        <div className="mb-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-red-500 to-rose-600 shadow-md shadow-red-500/20">
                          <Bot size={15} className="text-white" />
                        </div>
                        <div className="flex max-w-[88%] flex-col gap-2 rounded-2xl rounded-tl-sm border border-red-200 bg-red-50 px-5 py-4 text-sm shadow-sm dark:border-red-900/50 dark:bg-red-950/30">
                  ...
                  <AnimatePresence>
                    {isLoading && visibleMessages.at(-1)?.role !== 'assistant' && (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 rounded-2xl rounded-tl-sm border border-white/8 bg-white/4 px-5 py-4 shadow-sm backdrop-blur-sm">
                          <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-indigo-400/60 [animation-delay:-0.3s]" />
                          <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-indigo-400/60 [animation-delay:-0.15s]" />
                          <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-indigo-400/60" />
                        </div>
