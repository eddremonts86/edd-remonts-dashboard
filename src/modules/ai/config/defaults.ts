...
export const PROVIDER_DEFAULTS: Record<AiProvider, AiConfigFormData> = {
  minimax: {
    ...
    port: parsePort(MINIMAX_BASE_URL, 443),
    token: '',
...
export const normalizeConfig = (
  ...
  return {
    ...
    ...config,
    provider,
    token: config?.token || config?.apiKey || fallback.token || fallback.apiKey || '',
...
export const aiConfig: AiRuntimeConfig = {
  ...
  providers: {
    ...
    minimax: {
      ...
      defaultModel: getEnv('MINIMAX_MODEL', 'MiniMax-M2.7'),
      generation: { temperature: 0.7, maxTokens: 2048, topP: 1 },
    },
  },
}
